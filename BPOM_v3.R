...
simu <- function(scenario){
  for (i in 1:nrow(scenario)){
    ...
    if (!(is.null(model$value)|
          (is.null(modelPrior$value)))){
      model_coef <- summary(model$value)$coefficients %>% as.data.frame %>%
        rownames_to_column()
      model_coefPrior <- summary(modelPrior$value)$coefficients %>% as.data.frame %>%
        rownames_to_column()
    ...
    for (interim in 1:nInterim){
      ...
      if (!is.null(model1$value)){
        ...
        current_prior <- current_prior %>%
          left_join(new_stats, by = "rowname", suffix = c("", "_new")) %>%
          mutate(
            Estimate = coalesce(Estimate_new, Estimate),
            `Std. Error` = coalesce(`Std. Error_new`, `Std. Error`)
          ) %>%
          select(rowname, Estimate, `Std. Error`)
        
        # 5. Secondary Analysis (CVP)
        dfCVP <- dfRCT %>% filter(cAC > 1, groupT %in% c('A', 'C'), slice <= interim)
        model2 <- myTryCatch(glm(data = dfCVP, death ~ groupT + as.factor(combination), 
                                 family = binomial('identity')))
        
        # 6. Formatting Results
        res1 <- get_safe_res(model1, 'goodPrior', interim, scenario[i,], group_label = 'B', dfInterim = dfInterim)
        res2 <- get_safe_res(model2, 'CVP', interim, scenario[i, ], group_label = 'C', dfInterim = dfCVP)
        
        if(!is.null(res2)) res2 <- res2[str_detect(res2$term, 'groupT'), ]
        
        res <- rbind(res1, res2) %>%
          mutate(nA = sum(dfRCT$groupT == 'A'), nB = sum(dfRCT$groupT == 'B'))
...
resultSum %>% filter(model == 'goodPrior') %>% 
  arrange(p1, p2, priorSet, sampleSize) %>% 
  group_by(p1, p2, priorSet, sampleSize) %>% 
  summarise(power = 100*sum(nPOS)/sum(N_iteration),
            N_iteration = sum(N_iteration)) %>%
  mutate(mortalityPoly = paste0('Poly - BAT = ', 100*(p1 - p2), '%'),
         sampleSizeIND = sampleSize*0.2) %>%
  ggplot(data = ., aes(x = sampleSizeIND, y = power)) +
  geom_point(aes(shape = mortalityPoly, col = priorSet)) +
  facet_wrap(~mortalityPoly, ncol = 2) +
